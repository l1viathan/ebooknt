...
fz_lookup_base14_font(fz_context *ctx, const char *name, int *size)
{
	...
	if (!strcmp(name, "Courier")) {
		font_data = get_bytes_from_file(ext_font_Courier, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusMonoPS_Regular_cff);
	}
	if (!strcmp(name, "Courier-Oblique")) {
		font_data = get_bytes_from_file(ext_font_CourierOblique, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusMonoPS_Italic_cff);
	}
	if (!strcmp(name, "Courier-Bold")) {
		font_data = get_bytes_from_file(ext_font_CourierBold, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusMonoPS_Bold_cff);
	}
	if (!strcmp(name, "Courier-BoldOblique")) {
		font_data = get_bytes_from_file(ext_font_CourierBoldOblique, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusMonoPS_BoldItalic_cff);
	}
	if (!strcmp(name, "Helvetica")) {
		font_data = get_bytes_from_file(ext_font_Helvetica, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusSans_Regular_cff);
	}
	if (!strcmp(name, "Helvetica-Oblique")) {
		font_data = get_bytes_from_file(ext_font_HelveticaOblique, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusSans_Italic_cff);
	}
	if (!strcmp(name, "Helvetica-Bold")) {
		font_data = get_bytes_from_file(ext_font_HelveticaBold, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusSans_Bold_cff);
	}
	if (!strcmp(name, "Helvetica-BoldOblique")) {
		font_data = get_bytes_from_file(ext_font_HelveticaBoldOblique, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusSans_BoldItalic_cff);
	}
	if (!strcmp(name, "Times-Roman")) {
		font_data = get_bytes_from_file(ext_font_TimesRoman, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusRoman_Regular_cff);
	}
	if (!strcmp(name, "Times-Italic")) {
		font_data = get_bytes_from_file(ext_font_TimesItalic, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusRoman_Italic_cff);
	}
	if (!strcmp(name, "Times-Bold")) {
		font_data = get_bytes_from_file(ext_font_TimesBold, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusRoman_Bold_cff);
	}
	if (!strcmp(name, "Times-BoldItalic")) {
		font_data = get_bytes_from_file(ext_font_TimesBoldItalic, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,NimbusRoman_BoldItalic_cff);
	}
	if (!strcmp(name, "Symbol")) {
		font_data = get_bytes_from_file(ext_font_Symbol, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
		RETURN(urw,StandardSymbolsPS_cff);
	}
	if (!strcmp(name, "ZapfDingbats")) {
		font_data = get_bytes_from_file(ext_font_ZapfDingbats, &ext_len);
		if (font_data) { *size = (int)ext_len; return font_data; }
